...
export default function MaterialsCatalogPage() {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-6">
      <h1 className="text-3xl font-bold text-[#123b1f]">Materials Catalog</h1>
      ...
      <div className="flex items-center gap-3">
        <input className="border border-gray-200 p-2 rounded-lg text-sm flex-1" placeholder="Search materials…" value={search} onChange={e => setSearch(e.target.value)} />
      ...
      <div className="border border-gray-200 rounded-xl overflow-hidden bg-white shadow-sm">
        <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_auto] gap-3 px-4 py-2 bg-gray-50 border-b text-xs font-bold text-gray-500 uppercase tracking-wide">
          <div>Name</div>
          <div>Unit</div>
        ...
        {filtered.map(m => (
          <div key={m.id} className={`border-b last:border-0 ${!m.is_active ? "opacity-50" : ""}`}>
            {editingId === m.id ? (
