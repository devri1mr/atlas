...
export default function ReverseRevPage() {
  ...
  useEffect(() => {
    fetch("/api/atlas-time/employees", { cache: "no-store" })
      .then(r => r.json())
      .then(j => setEmployees((j.employees ?? []).filter((e: any) => e.default_pay_rate)))
      .catch(() => {});
  }, []);
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="mx-auto max-w-4xl">
        <div className="rounded-2xl overflow-hidden shadow-md border border-gray-200">
          ...
          >
            <div>
              <div className="text-base font-bold text-white">Reverse Revenue Calculator</div>
              <div className="text-xs text-white/40 mt-0.5">39% labor target · 15% payroll burden · +30 min lunch if ≥ 6 hrs</div>
            </div>
            <button
              onClick={addRow}
