...
function CostCell({ day, onSave }: { day: DayRecord; onSave: (date: string, val: number | null) => Promise<void> }) {
  ...
  return (
    <td
      className={`px-4 py-2.5 text-center cursor-pointer group relative ${saving ? "opacity-50" : ""}`}
      onClick={startEdit}
    >
...
const DEFAULT_CONFIG: Config = {
  manager_1_name: "", manager_2_name: "",
  manager_1_annual: null, manager_2_annual: null,
...
function SettingsPanel({ config, year, onSaved }: { config: Config | null; year: number; onSaved: () => void }) {
  ...
  useEffect(() => { setForm(config ?? DEFAULT_CONFIG); }, [config]);
  ...
  async function save() {
    ...
    try {
      const res = await fetch("/api/operations-center/atlas-ops/fertilization/admin-pay", {
        method: "PUT",
  ...
  return (
    <div className="rounded-xl border border-emerald-100 bg-white shadow-sm overflow-hidden mb-6">
      ...
      >
        ...
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={`text-gray-400 transition-transform ${open ? "rotate-180" : ""}`}><polyline points="6 9 12 15 18 9" /></svg>
      </button>
...
export default function FertAdminPayPage() {
  ...
  const load = useCallback(async (yr: number) => {
    setLoading(true);
    const res = await fetch(`/api/operations-center/atlas-ops/fertilization/admin-pay?year=${yr}`, { cache: "no-store" });
    const d = await res.json();
    setConfig(d.config);
    setDays(d.days ?? []);
    setLoading(false);
  ...
  async function handleOverride(date: string, val: number | null) {
    await fetch("/api/operations-center/atlas-ops/fertilization/admin-pay/override", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  if (!can("hr_labor_cost")) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-6">
        ...
        <p className="text-sm text-gray-500 max-w-xs">You don&apos;t have permission to view this page. Contact your administrator to request access.</p>
      </div>
    );
  }
  ...
  const ytdCost    = days.filter(d => d.date <= today).reduce((s, d) => s + effectiveCost(d), 0);
  const curMonth   = new Date().getMonth();
  const monthCost  = byMonth[curMonth].filter(d => d.date <= today).reduce((s, d) => s + effectiveCost(d), 0);
        ...
        <div className="flex items-end justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-emerald-900/60 mt-0.5">
              {config ? `${config.manager_1_name}${config.manager_2_name ? ` & ${config.manager_2_name}` : ""}` : "Division manager payroll allocation"}
            </p>
          </div>
                  ...
                  <table className="w-full text-sm border-collapse">
                    <thead>
                      <tr className="text-xs font-semibold text-emerald-900/50 bg-emerald-50/30">
                        <th className="px-5 py-2 text-center w-24">Day</th>
                    ...
                    <tbody>
                      {monthDays.map(d => {
                        ...
                        return (
                          ...
                          >
                            <td className={`px-5 py-2.5 text-sm text-center ${d.is_weekday ? "font-medium text-gray-700" : "text-gray-400"}`}>
                              {d.day_of_week}
                            </td>
                            <td className="px-5 py-2.5 text-sm text-gray-600 text-center">
