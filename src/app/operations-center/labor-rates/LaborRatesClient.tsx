...
export default function LaborRatesClient() {
  ...
  const filtered = React.useMemo(() => {
    ...
    return rates.filter((r) => {
      ...
      const rName = r.job_role_id ? roleName.get(r.job_role_id) ?? "" : "";
  ...
  function isRowEdited(row: LaborRateRow) {
    ...
    const roleChanged = roleDraft !== undefined && (roleDraft === "" ? null : roleDraft) !== row.job_role_id;

    return rateChanged || divChanged || roleChanged;
  }
  ...
  async function saveRow(row: LaborRateRow) {
    ...
    try {
      ...
      const roleDraft = draftRole[id];
      if (roleDraft !== undefined && roleDraft !== "") payload.job_role_id = roleDraft;

      // If user never touched dropdowns, preserve existing
      if (payload.division_id === undefined && typeof row.division_id === "number") payload.division_id = row.division_id;
      if (payload.job_role_id === undefined && typeof row.job_role_id === "number") payload.job_role_id = row.job_role_id;
  ...
  async function createRow() {
    if (newDivisionId === "" || newRoleId === "") {
      setToast({ id: crypto.randomUUID(), kind: "error", title: "Select a division and role" });
      return;
    }
    ...
    <div className="max-w-[1120px] mx-auto px-4 py-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1">Edit hourly rates like a spreadsheet. Save per row.</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Dialog.Root open={addOpen} onOpenChange={setAddOpen}>
            ...
            <Dialog.Portal>
              {/* FIX: ensure modal is ALWAYS above header */}
      ...
      <div className="mt-4 rounded-xl border border-slate-200 bg-white">
        <div className="grid grid-cols-[1.2fr_1.2fr_1fr_220px] gap-0 border-b border-slate-200 bg-slate-50 px-4 py-3 text-xs font-semibold uppercase text-slate-600">
          <div>Division</div>
          <div>Role</div>
          <div>Hourly Rate</div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100">
            {filtered.map((row) => {
              ...
              const roleValue =
                draftRole[id] !== undefined
                  ? draftRole[id]
                  : typeof row.job_role_id === "number"
                    ? row.job_role_id
                    : "";
              ...
              return (
                <div key={id} className="grid grid-cols-[1.2fr_1.2fr_1fr_220px] items-center gap-0 px-4 py-3">
                  <div className="pr-3">
                    <select
                  ...
                  <div className="pr-3">
                    ...
                    {edited && <div className="mt-1 inline-flex rounded bg-emerald-50 px-2 py-0.5 text-xs text-emerald-800">Edited</div>}
                  </div>
                  ...
                  <div className="flex items-center justify-end gap-2">
                    ...
                    >
                      {savingId === id ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}
                      Save
                    </button>
        ...
        <div className="flex items-center justify-between px-4 py-3 text-xs text-slate-500">
          <div>Tip: edit cells → click <b>Save</b> on that row.</div>
          <div>All editing enabled.</div>
        </div>
      </div>

      {/* Delete confirm dialog - CLEAN (no extra division/role text) */}
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 z-[99999] w-[92vw] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl bg-white p-5 shadow-2xl focus:outline-none">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <Dialog.Description className="mt-1 text-sm text-slate-600">This cannot be undone.</Dialog.Description>
              </div>
              <Dialog.Close asChild>
                <button className="rounded-md p-2 text-slate-500 hover:bg-slate-100">
                  <X className="h-4 w-4" />
      ...
      {toast && (
        <div className="fixed bottom-6 right-6 z-[100000] w-[360px] max-w-[90vw] rounded-xl border border-slate-200 bg-white p-4 shadow-xl">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className={`text-sm font-semibold ${toast.kind === "error" ? "text-red-700" : "text-emerald-700"}`}>
                {toast.title}
              </div>
