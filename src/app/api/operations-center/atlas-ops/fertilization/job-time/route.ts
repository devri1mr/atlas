...
// ── GET /api/operations-center/atlas-ops/fertilization/job-time?job_id=xxx ──────────────
...
// ── PATCH /api/operations-center/atlas-ops/fertilization/job-time ───────────────────────
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    for (const m of members) {
      ...
      if (timeVaries) {
        ...
        if (m.start_time && m.end_time) {
          if (m.dispatch_time_id) {
            await sb.from("fert_dispatch_job_times")
      ...
      await sb.from("fert_production_members")
    ...
    if (!timeVaries && sharedDispatchJobId && members[0]?.start_time && members[0]?.end_time) {
      await sb.from("fert_dispatch_jobs")
    ...
    await sb.from("fert_production_jobs")
      .update({
