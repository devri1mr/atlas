...
function lrm(weights: number[], total: number, decimals: number): number[] {
  ...
  const order = exact.map((v, i) => ({ i, frac: (v * factor) % 1 })).sort((a, b) => b.frac - a.frac);
  const unit  = 1 / factor;
...
type RawMember = { resource_name: string; resource_code: string; actual_hours: number; earned_amount: number };

type RawJob = {
  work_order: string; client_name: string; client_address: string;
  service: string; service_date: string; crew_code: string;
  budgeted_hours: number; actual_hours: number; accumulated_hours: number;
  variance_hours: number; budgeted_amount: number; actual_amount: number;
  is_dispatched: boolean; members: RawMember[];
};

type ParsedMember = RawMember & {
  employee_id: string | null; employee_name: string | null;
  punch_status: PunchStatus; reg_hours: number | null; ot_hours: number | null;
  total_payroll_hours: number | null; pay_rate: number | null; payroll_cost: number | null;
  clock_in_at: string | null; clock_out_at: string | null;
};

type ParsedJob = Omit<RawJob, "members"> & { members: ParsedMember[] };
...
function parseXLS(buffer: Buffer) {
  ...
  if (grandTotalHrs !== null && jobs.length > 0) {
    ...
    jobs.forEach((j, i) => {
      ...
      if (j.members.length > 0) {
        ...
        j.members.forEach((m, k) => { m.actual_hours = mHrs[k]; m.earned_amount = mRev[k]; });
      }
  ...
  jobs.forEach(j => {
    if (j.is_dispatched) { j.budgeted_amount = 0; j.actual_amount = 0; j.members.forEach(m => { m.earned_amount = 0; }); }
  });

  return {
    jobs, revenueOk, grandTotalAmt, sumJobAmt,
    dispatchedCount: jobs.filter(j => j.is_dispatched).length,
    debug: { grandTotalHrs, grandTotalAmt, sumJobAmt, revenueOk, totalRowFound: totalRowRaw !== null },
  };
}
...
type DispatchJob = {
  work_order: string | null; client_name: string; address: string; city: string;
  zip: string; service: string; crew_code: string; personnel_count: number | null;
  report_date: string; start_time: string | null; end_time: string | null; time_varies: boolean;
};
...
function parseTimeString(timeVal: unknown, dateISO: string, tzOffsetMins: number): string | null {
  ...
  return new Date(localMs + tzOffsetMins * 60 * 1000).toISOString();
}
...
function parseDispatchXLS(buffer: Buffer, tzOffsetMins: number): DispatchJob[] {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (typeof dateVal === "number" && dateVal > 40000) {
      ...
      if (parts.length === 3) {
        const [mo, d, y] = parts;
        reportDate = `${y}-${mo.padStart(2, "0")}-${d.padStart(2, "0")}`;
      }
    }
...
async function handleDispatchImport(
  ...
  if (dryRun) {
    return NextResponse.json({ report_type: "dispatch", file_name: fileName, report_date: reportDate, jobs, varies_count: variesCount });
  }

  if (reportDate) {
    await sb.from("fert_dispatch_jobs").delete().eq("company_id", companyId).eq("report_date", reportDate);
  }

  const { error } = await sb.from("fert_dispatch_jobs").insert(
    jobs.map(j => ({
      company_id: companyId, report_date: j.report_date || reportDate,
      work_order: j.work_order, client_name: j.client_name,
      address: j.address || null, city: j.city || null, zip: j.zip || null,
      service: j.service || null, crew_code: j.crew_code || null,
      personnel_count: j.personnel_count,
      start_time: j.start_time, end_time: j.end_time, time_varies: j.time_varies,
    }))
  );
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (isDispatch) return handleDispatchImport(sb, companyId, buffer, file.name, dryRun, tzOffsetMins);
    ...
    const matchedEmpIds = [...new Set([...matchCache.values()].filter(Boolean).map(v => v!.id))];

    // ── Punch data ───────────────────────────────────────────────────────────
    type PunchRow = { employee_id: string; clock_in_at: string | null; clock_out_at: string | null; regular_hours: number | null; ot_hours: number | null; dt_hours: number | null };
    ...
    // Punch status: matched = has a Fert punch on the report date
    ...
    if (matchedEmpIds.length > 0) {
      const { data } = await sb.from("at_pay_rates").select("employee_id, division_id, rate, is_default").in("employee_id", matchedEmpIds);
    ...
    // ── OT settings ─────────────────────────────────────────────────────────
    const { data: atSettings } = await sb.from("at_settings").select("ot_multiplier, dt_multiplier").eq("company_id", companyId).maybeSingle();
    ...
    const resolvedJobs: ParsedJob[] = rawJobs.map(j => ({
      ...
      members: j.members.map(m => {
        ...
        const baseCost            = (hrs && rate) ? hrs.reg * rate + hrs.ot * rate * otMult + hrs.dt * rate * dtMult : null;
        const payroll_cost        = baseCost !== null ? Math.round(baseCost * PAYROLL_BURDEN * 100) / 100 : null;
        ...
        return { ...m, employee_id: empId, employee_name: match?.name ?? null, punch_status: getPunchStatus(empId), reg_hours, ot_hours, total_payroll_hours, pay_rate: rate, payroll_cost, clock_in_at, clock_out_at };
      }),
    }));

    if (dryRun) {
      const empNameMapDry = new Map<string, string>();
      for (const j of resolvedJobs) for (const m of j.members) if (m.employee_id) empNameMapDry.set(m.employee_id, m.resource_name);
      return NextResponse.json({
        jobs: resolvedJobs, file_name: file.name,
        revenue_ok: revenueOk, revenue_imported: sumJobAmt, revenue_expected: grandTotalAmt,
        dispatched_count: dispatchedCount,
        punches: punchRows.map(p => ({ employee_id: p.employee_id, resource_name: empNameMapDry.get(p.employee_id) ?? "", clock_in_at: p.clock_in_at, clock_out_at: p.clock_out_at, regular_hours: p.regular_hours, ot_hours: p.ot_hours })),
    ...
    const { data: report, error: repErr } = await sb.from("fert_production_reports")
      .insert({ company_id: companyId, report_date: reportDate ?? estToday(), file_name: file.name, total_budgeted_hours: totalBudgHrs, total_actual_hours: totalActHrs, total_budgeted_amount: totalBudgAmt, total_actual_amount: totalActAmt })
      .select("id").single();
    ...
    for (const j of resolvedJobs) {
      ...
      const { data: job, error: jobErr } = await sb.from("fert_production_jobs")
        .insert({ report_id: report.id, work_order: j.work_order || null, client_name: j.client_name, client_address: j.client_address || null, service: j.service || null, service_date: j.service_date || null, crew_code: j.crew_code || null, budgeted_hours: j.budgeted_hours, actual_hours: j.actual_hours, accumulated_hours: j.accumulated_hours || null, variance_hours: j.variance_hours, budgeted_amount: j.budgeted_amount, actual_amount: j.actual_amount, real_budgeted_hours: realBudgetedHours, status: j.is_dispatched ? "dispatched" : "completed" })
        .select("id").single();
      if (jobErr) continue;

      if (j.members.length) {
        await sb.from("fert_production_members").insert(
          j.members.map(m => ({ job_id: job.id, resource_name: m.resource_name, resource_code: m.resource_code || null, employee_id: m.employee_id ?? null, actual_hours: m.actual_hours, earned_amount: m.earned_amount, punch_status: m.punch_status, reg_hours: m.reg_hours, ot_hours: m.ot_hours, total_payroll_hours: m.total_payroll_hours, pay_rate: m.pay_rate, payroll_cost: m.payroll_cost }))
        );
      }
    }
    ...
    if (punchRows.length > 0) {
      ...
      for (const j of resolvedJobs) for (const m of j.members) if (m.employee_id) empNameMap.set(m.employee_id, m.resource_name);
      await sb.from("fert_report_punches").insert(
        punchRows.map(p => ({ report_id: report.id, employee_id: p.employee_id, resource_name: empNameMap.get(p.employee_id) ?? "", clock_in_at: p.clock_in_at, clock_out_at: p.clock_out_at, regular_hours: p.regular_hours, ot_hours: p.ot_hours, dt_hours: null }))
      );
    }
