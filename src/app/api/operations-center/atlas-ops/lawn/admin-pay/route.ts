...
// Compute the daily rate for a given month based on config
function computeDailyRate(config: any, year: number, monthIndex: number): number {
  ...
  // If there's an explicit monthly daily rate override, use it
  if (config[key] != null) return Number(config[key]);
  // Otherwise compute from annual salaries
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Load all overrides for this year
    ...
    // Build day records for the entire year
    const days: Array<{
      date: string;
      day_of_week: string;
      is_weekday: boolean;
